...
# Optional channels: included in the output ONLY if the log has them.
# (current: SpeedyBee ESC current sensor; rssi: ELRS link stats via CRSF)
OPTIONAL_ALIASES = {
    "current_a": ["amperage (A)", "amperage", "currentLatest"],
    "rssi": ["rssi", "rxRSSI", "rssi_dbm"],
}
...
def standardize(raw: pd.DataFrame, report: list) -> pd.DataFrame:
    ...
    # optional measured channels
    lookup = {_norm(c): c for c in raw.columns}
    for field, aliases in OPTIONAL_ALIASES.items():
        hit = next((lookup[_norm(a)] for a in aliases if _norm(a) in lookup), None)
        if hit is not None:
            out[field] = pd.to_numeric(raw[hit], errors="coerce")
            report.append(f"optional: found {field} (column '{hit}')")
    ...
    rounding = {"lat": 7, "lon": 7, "alt_m": 2, "ground_speed_ms": 2,
                "roll_deg": 2, "pitch_deg": 2, "yaw_deg": 1, "vbat_v": 2,
                "current_a": 2, "rssi": 1}
    out = out.round({k: v for k, v in rounding.items() if k in out.columns})
